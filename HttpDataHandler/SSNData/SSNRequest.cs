...
namespace HttpDataHandler.SSNData
{
    ...
    {
        private string urlString = "http://www.haoweichi.com";
        /// <summary>
        /// 连接的网址
        /// </summary>
        ...
        private string selectGEO = String.Empty;
        public override string SelectGEO
        {
            get
            {
                return selectGEO;
            ...
            {
                selectGEO = value;
                URLString = GeoCategoryList.Find(geo => geo.Key == selectGEO).Value;
            }
        }
        ...
        protected override CL.Model.SSN GetSSNByHTML(string html)
        {
            ...
            CL.Model.SSN model = new CL.Model.SSN();
            model.Country = SelectGEO;
            ...
            {
                //Middle name
                int tempStartIndex = html.IndexOf("Middle name");
                if (tempStartIndex > -1)
                {
                    string tempStr = html.Substring(tempStartIndex);
                    var m = Regex.Match(tempStr, "<input type=\"text\" value=(('.*' )|(\".*\" ))");
