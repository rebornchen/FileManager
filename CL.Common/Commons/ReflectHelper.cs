...
namespace CL.Common
{
    ...
    public static class ReflectHelper
    {
        ...
        public static Type GetTypeByObjectName(string path, string className)
        {
            //动态从程序集中查找所需要的类并使用系统激活器创建实例最后获取它的Type
            Type type = System.Reflection.Assembly.Load(path).CreateInstance(path + "." + className).GetType();
            return type;
        }
        #endregion
